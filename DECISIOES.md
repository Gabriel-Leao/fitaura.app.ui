...
- **Ciclo de vida de pedidos:** status atualizado automaticamente por tempo quando o servidor não responde — cancelado após 20s em `received`, atrasado após 30s e falha após 60s. Ao reabrir o app, pedidos salvos no AsyncStorage passam por resolução retroativa com base no `placedAtISO`, corrigindo estados travados de sessões anteriores.
- **Fluxo de reembolso:** solicitado via evento Socket.IO `request_refund`, que percorre os estados `refund_requested` → `refund_processing` → `refunded` pelo servidor, espelhando o mesmo padrão do `place_order`. Quando offline, o mesmo fluxo é simulado localmente com timers.
